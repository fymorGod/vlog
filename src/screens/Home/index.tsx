...
export function Home() {
  ...
  function handleScan({ data }: BarCodeScannerResult) {
    if (data) {
      setLoading(true); // Definindo loading como true ao iniciar a operação de scan
  ...
  function sendNFEManualmente() {
    ...
    if (textValid.length == 44) {
      setLoading(true); // Definindo loading como true ao iniciar o envio manual
  ...
  return (
    <View style={styles.container}>
      {loading && ( // Renderizando a tela de carregamento apenas quando loading for true
